...
# 지정된 폴더에 가서 이미지를 가져오고 가져온 이미지를 행열배열(독립)과 값배열(종속)로 반환
...
def train_model_save_model(X, y, file_name:str = 'mnist_model_pkl'):
  # 데이터를 섞음
  X, y = shuffle(X,y)
...
def load_model_predict(images:str, width:int, height:int, file_name:str = 'mnist_model_pkl'):
...
def predict_from_upload_file(img, width:int, height:int, file_name:str = 'mnist_model_pkl'):
